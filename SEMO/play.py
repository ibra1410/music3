...
async def aii(client: Client, message):
   ...
   try:
    ...
    chat_id = message.chat.id
    user_id = message.from_user.id if message.from_user else "MeDoO152"
    videoid = None
    link = None
     ...
     if await is_served_call(client, message.chat.id):
         ...
         if message.from_user:
          if message.from_user.photo:
           ...
           MeDoO = await client.get_chat("MeDoO152")
           MeDoOphoto = MeDoO.photo.big_file_id
         elif message.chat.photo:
          photo_id = message.chat.photo.big_file_id
          photo = await client.download_media(photo_id)
         else:
          MeDoO = await client.get_chat("MeDoO152")
          MeDoOphoto = MeDoO.photo.big_file_id
          photo = await client.download_media(MeDoOphoto)
